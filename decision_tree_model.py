...
from imblearn.over_sampling import SMOTE, RandomOverSampler # Added RandomOverSampler as it was in source, though SMOTE is used
...
def create_decision_tree_pipeline(sampling_method=None):
    ...
    # Add other sampling methods if needed, e.g., RandomOverSampler
    
    steps.append(('model', DecisionTreeClassifier(random_state=RANDOM_STATE)))
    return ImbPipeline(steps)

if __name__ == "__main__":
    print("\nRunning Decision Tree Model (with SMOTE)...\n")
    
    # Create and train the Decision Tree model with SMOTE
    dt_pipeline_smote = create_decision_tree_pipeline(sampling_method='smote')
    
    print("Training Decision Tree (SMOTE)...")
    dt_pipeline_smote.fit(X_train, y_train)
    print("Training complete.")
    
    print("\nEvaluating Decision Tree (SMOTE)...")
    evaluate_model(dt_pipeline_smote, X_test, y_test, model_name="Decision_Tree_SMOTE_Standalone")
    print("\nDecision Tree (SMOTE) evaluation complete.") 